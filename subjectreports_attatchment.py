    ...
    if not api_key:
        ...
        with st.spinner("1학기 내용을 분석하고 2학기 내용을 작성 중입니다..."):
            try:
                ...
                # 에러 처리
                if "429" in str(e):
                    st.error("🚨 사용량이 많아 잠시 멈췄습니다. (잠시 후 다시 시도하거나 Pro 모델을 끄세요)")
                elif "404" in str(e):
                    st.error("🚨 모델을 찾을 수 없습니다. (터미널에 'pip install -U google-generativeai'를 입력해 업데이트하세요)")
                else:
                    st.error(f"오류 발생: {e}")
